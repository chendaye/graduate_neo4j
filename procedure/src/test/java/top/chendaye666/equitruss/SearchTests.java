...
public class SearchTests {
    ...
    public void searchtime() {
        try(Session session = driver.session()) {
            session.run(MODEL_STATEMENT);
            final Result result = session.run("MATCH (u:Author {authorId:'4'})  CALL top.chendaye666.equitruss.time(u,4,4,1,2000) YIELD community,article RETURN community,article");
            while (result.hasNext()){
                Record next = result.next();
                Iterator<Value> iterator = next.values().iterator();
                String cur = "";
