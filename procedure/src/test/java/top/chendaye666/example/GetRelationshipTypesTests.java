...
public class GetRelationshipTypesTests {
    ...
    public void shouldReturnTheTypesWhenThereIsOneEachWay() {
        ...
        try(Session session = driver.session()) {
            ...
            Record record = session.run("MATCH (u:Movie {id:1}) CALL top.chendaye666.example.getRelationshipTypes(u) YIELD outgoing, incoming RETURN outgoing, incoming").single();
    ...
    public void shouldReturnEmptyStreamIfInputNodeIsNull(){
        try(Session session = driver.session()) {
            Result result = session.run("MATCH (u:Movie {id:1}) CALL top.chendaye666.example.getRelationshipTypes(u) YIELD outgoing, incoming RETURN outgoing, incoming");
