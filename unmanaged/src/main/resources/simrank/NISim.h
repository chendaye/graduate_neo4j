...
public:
    NISim(double decayfactor, int *orig_graph_src, int *orig_graph_dst, int rank, int mvid, bool isinit,
        ...
        sprintf(Upath, "%s/%s/index/nisim/OptKron/%s.U", BASE_PATH, graphName, graphName);
        sprintf(V_rpath, "%s/%s/index/nisim/OptKron/%s.V_r", BASE_PATH, graphName, graphName);
        sprintf(Apath, "%s/%s/index/nisim/OptKron/%s.A", BASE_PATH, graphName, graphName);
    }
